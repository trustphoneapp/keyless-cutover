...
- Fifty-three passing local tests, including a simultaneous replay race, the Cloud Run canary and evidence-console contracts, documentation-link integrity, exact cutover compilation, immutable WIF trust/readback planning, strict ADK invocation/output/citation contracts, three-repeat deterministic evaluation thresholds, Firestore transitions, GitHub observation, authenticated Google key/audit lookup, hostile and fresh-legacy denial collection, crash-window-aware draft PR creation, the reviewed ProofV2 operator transaction, and offline evidence reconstruction.
...
- PR #14 merged the hardened reviewed ProofV2 workflow at `9e35d3d40f8cb3f0b1f8185226b1118ee5355acc`; post-merge CI passed, `production` requires reviewer `cherala2002` with self-review and admin bypass disabled, and the exact legacy key remains enabled. A new five-minute challenge/run/approval/consume transaction is still required before this becomes release evidence.
...
- A fresh independently approved ProofV2 transaction on the merged workflow, complete hostile-matrix reconstruction, human key disable, fresh legacy rejection, post-disable `wif-2`, KMS receipt, or video.
