...
The agent HTTP routes require a bearer secret of at least 256 bits; only `/_health` is public. Store the token in Secret Manager at deployment, never in the image or repository. The service rejects bodies over 64 KiB and the model contract further caps evidence text at 32,000 characters.
