...
Current checkpoint on August 25: a second, independent fresh disposable key (`851d4503…`) repeated the entire transaction, deliberately testing the August 24 finding's fix. The fix worked exactly as predicted — gating the hostile jobs behind `KEYLESS_K0_POST_DISABLE_PUSH` correctly skipped h6/h7/h8 on the post-disable push, confirmed live, because this transaction's own `wif-1` deploy pinned bytes that already included the gate from the start. Legacy baseline, ProofV2, the WIF cutover, `wif-1`, and H1–H8 all completed; the canonical pre-disable archive checkpoint was reviewed and merged before the key was disabled, the correct ordering proven a second time. The key was disabled by a human and proven unable to authenticate afterward. The post-disable push then deployed live through WIF with no key involved. That transaction also produced the finding that unblocked the rest of the work: isolating the audit window to the deploy job alone, with every hostile job skipped, showed three Cloud Audit Log entries where the checks expected two — the authentication action performs its own exchange, and `gcloud run deploy` independently performs another when it calls the Cloud Run API. Recording the real bytes then showed three further places where the checks were pinned to Google's published example shapes rather than what Google actually logs.

Both are now addressed on `main`. The deploy step takes an access token from the authentication action and calls the Cloud Run API directly, producing the two entries the checks expect, and the audit assertions are pinned to observed reality with every relaxed shape paid for by a stronger value binding. All 10 genuine historical exchanges now parse where none could before, and 10 forgery attempts are still refused, all covered by tests driven from the recorded bytes in `docs/evidence/forensics/`. See `README.md`'s "The second transaction" section for the complete account with run IDs and timestamps. Archive-before-disable ordering is proven twice independently; end-to-end certification is one transaction away on the corrected checks.
