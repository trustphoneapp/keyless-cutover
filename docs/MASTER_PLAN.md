...
Current checkpoint on August 24: PRs #18 and #19 are merged on protected `main`, `required_linear_history: true` is enforced, and the complete branch/environment protection tuple was read back on August 15. Since then PR #27 restored the canonical cutover path, the compiler-owned WIF cutover waits as draft PR #28 until a fresh legacy baseline exists, `KEYLESS_K0_ENABLED` was deliberately set to `false` as a kill switch, `sts.googleapis.com` data-access audit logging was enabled on the project, and the pre-disable collectors gained a real two-command executable. None of that is live transaction evidence. The live Google/GitHub substrate, historical WIF cutover, ProofV2, H1–H8, public evidence console, and exact historical key disable remain historical readiness only: that key was disabled before a canonical v3 pre-disable archive checkpoint was reviewed and merged, so that transaction cannot satisfy v3 and must never be resumed by re-enabling the key. Local v3 bundle assembly, exact loading, authenticated read-only pending issuance, pending-receipt reconstruction, public signature verification, and false-safe tests are published, but none of that is a completed live v3 transaction. A fresh enabled disposable key exists and is not yet a K0 result, and its three release markers are not pinned: `legacy-1`, `legacy-2`, and `wif-1` are burned on `keyless-demo` and `demo/release.txt` still holds the burned `wif-1`. Overall status remains **NO-GO** pending the separately authorized fresh key transaction.
