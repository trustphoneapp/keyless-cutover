...
Current checkpoint on August 24: a fresh disposable key (`1f0137c5…`) ran the transaction through legacy denial, all against real GitHub and Google state. Legacy baseline, ProofV2, the WIF cutover, `wif-1`, and H1–H8 all completed; the canonical pre-disable archive checkpoint was reviewed and merged, with its required check and push run both completing before the key was disabled — the exact ordering the historical August transaction failed. The key was disabled by a human and proven unable to authenticate afterward. Continuity (`wif-2`) could not be completed: live testing found the hostile probes sharing `deploy`'s trigger always add extra audit-log entries to the post-disable window, and the only fix (gating them behind a variable) changes the workflow's bytes, which breaks the separate requirement that the post-disable run's bytes match what `wif-1` used. A second attempt with a new key hits the same wall from the other side: a fresh legacy baseline can never be collected again on this repository, because it requires the canonical workflow path to simultaneously carry the frozen legacy bytes, which was only true before the one-time cutover merged. See `README.md`'s "The fresh transaction" section for the complete account with run IDs and timestamps. Overall status remains **NO-GO**: archive-before-disable ordering is proven; continuity is not, for a now-understood structural reason rather than a remaining task.
