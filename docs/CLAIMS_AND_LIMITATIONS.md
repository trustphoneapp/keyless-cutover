...
Continuity — a fresh post-disable `wif-2` deployment — was not achieved in that transaction, and live testing found the hostile jobs sharing `deploy`'s trigger always added extra Cloud Audit Log entries to the post-disable window.

A second, independent fresh transaction on 2026-08-25 (key `851d4503…`) deliberately tested the fix for that exact finding: because this transaction's own `wif-1` deploy pinned bytes that already carried the trigger-gating fix from the start, the post-disable push correctly skipped the hostile jobs, confirmed live. `keyless-demo-wif-6` deployed through WIF with no key involved, after the key was disabled. But the audit window, isolated to just the deploy job with every hostile job skipped or left unapproved, still produced three Cloud Audit Log entries, not the required two: one from the authentication action's own credential-setup exchange, and one pair from `gcloud run deploy`'s own independent exchange when it actually calls the Cloud Run API. This is inherent to how this authentication action and `gcloud` combine on any deploy, not an artifact of hostile interference. See [README.md](../README.md)'s "The second transaction" section for exact run IDs and timestamps.

Authenticated pending issuance and the scoped KMS signature were correctly not attempted in both transactions, since both require `post_disable` evidence that cannot exist without a certified `wif-2`; the manifest remains `RECOLLECTION_REQUIRED` with `release_ready: false`. A Cloud KMS keyring, signing key, and scoped signer service account were provisioned during the second transaction and remain available. The overall claim therefore remains unproven: one real, now-fixed cause is confirmed resolved, and a second, more permanent structural cause in the authentication mechanism itself now blocks continuity's certification.
...
As of 2026-08-25, confirmed twice independently: "Exact old key identified", "No privilege widening", "Authorized WIF works", "Hostile path denied", "Key disabled", and "Fresh old-key auth fails" are met. "Pre-disable transaction checkpointed" is only partially met: the archive was independently reviewed and merged before disable in both transactions, but its "reread" component runs inside the pending issuer, which never ran. "WIF continuity" is not met, now for a second, structural reason beyond the first transaction's trigger-sharing bug: even a hostile-free deploy through this authentication action and gcloud produces three audit-log entries, not two. "Receipt authentic" remains correctly not attempted, since it requires the post_disable evidence that continuity's block prevents from existing.
