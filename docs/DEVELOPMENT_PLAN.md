...
Current progress on August 13: the public repository is live, its first hosted CI run passed, secret scanning/push protection and Dependabot security updates are enabled, and `main` requires the `test` status, one non-last-pusher approval, linear history, and resolved conversations. The `production` environment and an explicit `KEYLESS_K0_ENABLED=false` kill switch exist. The disposable project `keyless-k0-20260813` exists unbilled and is the active CLI project; Google enabled its standard project service set automatically, but no K0 resource has been created. The deterministic compiler/WIF plan portions of C4, the tool-free ADK schemas of C5, and the 36-case corpus/scorer/lockfile portions of C9 are implemented. C2 has local Firestore, GitHub-observer, and Google-key-reader adapters with deterministic tests, but no live integration pass. No live Gemini result, Cloud Run ADK deployment, or sealed-case pass is claimed yet.
